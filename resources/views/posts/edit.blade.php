...
@section('content')
    ...
    <form action="{{ route('posts.update', $post->id) }}" method="post" class="form-control">
        ...
        <div class="form-floating mt-3">
            <textarea name="content" class="form-control" style="height: 200px"
                      id="floatingTextarea">{{ old('content', $post->content) }}</textarea>
        ...
        <div class="mt-3">
            ...
            <select name="category_id" class="form-select" id="select-cat">
                @foreach ($categories as $el)
                    <option
                        {{ $el->id === (int)old('category_id', $post->category_id) ? ' selected' : '' }} value="{{ $el->id }}">{{ $el->name }}</option>
                @endforeach
            </select>
        </div>
        <div class="mt-3">
